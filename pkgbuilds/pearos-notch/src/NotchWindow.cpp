...
// [pearOS-ui-fix] X11: set _NET_WM_STRUT_PARTIAL over exactly the notch strip,
// so maximized/fullscreen windows can never slide under or collide with the
// notch. Mirrors macOS behavior where the menu-bar/notch area is reserved.
// Wayland already handles this correctly via layer-shell.
void NotchWindow::updateX11Strut() {
    if (isWayland()) return;
    auto* x11 = qApp->nativeInterface<QNativeInterface::QX11Application>();
    if (!x11) return;
    xcb_connection_t* conn = x11->connection();
    ...
    // Reserve only the collapsed strip height; the expanded island may be
    // overlapped by nothing anyway (it is transient), matching macOS.
    int gap = (Config::instance().mode() == "island") ? Config::instance().screenGap() : 0;
    int reservedBottom = g.y() + Config::instance().smallH() + gap;

    uint32_t strut[4]      = {0, 0, (uint32_t)reservedBottom, 0};          // L R T B
    uint32_t partial[12]   = {0, 0, (uint32_t)reservedBottom, 0, 0, 0, 0, 0, 0, 0, 0, 0};
    partial[8]  = (uint32_t)g.x();                                          // top_start_x
    partial[9]  = (uint32_t)(g.x() + g.width() - 1);                        // top_end_x
    ...
    xcb_atom_t aStrut   = intern("_NET_WM_STRUT");
    xcb_atom_t aPartial = intern("_NET_WM_STRUT_PARTIAL");
    if (aStrut)
        xcb_change_property(conn, XCB_PROP_MODE_REPLACE, win, aStrut, XCB_ATOM_CARDINAL, 32, 4, strut);
    if (aPartial)
        xcb_change_property(conn, XCB_PROP_MODE_REPLACE, win, aPartial, XCB_ATOM_CARDINAL, 32, 12, partial);
    xcb_flush(conn);
}
...
void NotchWindow::showEvent(QShowEvent* e) {
    ...
    else { setX11Hints(); updateX11Strut(); }   // [pearOS-ui-fix] reserve notch strip
    disableKWinEffects();
}
...
#endif
    move(x, y);
    updateX11Strut();   // [pearOS-ui-fix] re-assert strut after every reposition
}
